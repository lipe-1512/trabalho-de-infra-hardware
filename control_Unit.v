module control_Unit (
    ...
    // Inicialização na declaração para evitar 'X' no tempo 0 ps
    ...
    output reg [3:0] mem_reg = 4'b0000,
    output reg [2:0] reg_dst = 3'b000,
    ...
    output reg [3:0] load_control = 4'b0000,
    output reg [3:0] store_control = 4'b0000,
    output reg mult_start = 1'b0,
    output reg div_start = 1'b0,
    output reg [1:0] mult_div_sel_lo = 2'b00,
    output reg [1:0] mult_div_sel_hi = 2'b00,
    output reg Lo_wr = 1'b0,
    output reg Hi_wr = 1'b0,
    output reg reset_out = 1'b1,
    output reg [1:0] shift_control_in = 2'b00,
    output reg [2:0] shift_control = 3'b000,
    output reg [1:0] shift_n = 2'b00
);
// Inicialização explícita das variáveis de estado e contador
...
always @(posedge clk or posedge reset) begin
    if (reset == 1'b1) begin
        ...
        case (state)
            RESET_START: begin
                reset_out <= 1'b1;
                reg_dst <= 3'b011; mem_reg <= 4'b0111; reg_wr <= 1'b1;
            ...
            LW: begin
                if (counter == 6'b000000) begin
                    ...
                    mem_wr <= 1'b0; load_control <= 4'b0010;
